...
import '../../profile/profileModel/simpleUserProfileModel.dart' hide VideoTypes;
...
class ProfessionalProfileController extends GetxController {
  ...
  Future<void> getUserDetails() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        userDetails.value = await compute(
        ...
        totalLikes.value = _sumVideoLikes(userDetails.value?.videoTypes);
  ...
  // ---------------------------------------------------------------------------
  // Real-time upload processing watcher (mirrors ProfileController): flips the
  // just-uploaded "processing" grid tile to a playable poster the moment the
  // server transcode is watch-ready, without a pull-to-refresh.
  ...
  Set<String> _collectProcessingVideoIds() {
    final ids = <String>{};
    final types = userDetails.value?.videoTypes;
    if (types == null) return ids;
    for (final type in types) {
      for (final v in type.videos ?? const <ProfessionalVideos>[]) {
  ...
  Future<void> updateUserProfile({
    ...
    try {
      ...
      if (menuId != -1) fields['business_type'] = menuId.toString();
      ...
      if (latitude != null && latitude.isNotEmpty) fields['latitude'] = latitude;
  ...
  Future<bool> updateCoverImage({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          var errorData = jsonDecode(responseData);
          errorMessage = "Cover image update failed";
