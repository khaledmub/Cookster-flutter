...
import 'package:media_kit_video/src/video_controller/android_surface_size_gate.dart';
    ...
    videoParamsSubscription = player.stream.videoParams.listen(
      (event) => lock.synchronized(() async {
        ...
        if (event.rotate == 0 || event.rotate == 180) {
          ...
          rawWidth = event.dh ?? 0;
          rawHeight = event.dw ?? 0;
        }

        final currentW = rect.value?.width.toInt();
        final currentH = rect.value?.height.toInt();
        // COOKSTER: clamp odd/even parity BEFORE SetSurfaceSize so ImageReader
        // is never torn down for 720↔721-style reports.
        final gated = AndroidSurfaceSizeGate.gate(
          width: rawWidth,
          height: rawHeight,
          currentWidth: currentW,
          currentHeight: currentH,
        );
        final width = gated.width;
        final height = gated.height;

        final isZero = width == 0 || height == 0;
        final isSame =
            width == currentW && height == currentH;
        if (gated.adjusted) {
          final action = isSame || isZero
              ? 'skip_setSurfaceSize'
              : 'clamp_before_setSurfaceSize';
          debugPrint(
            '[ReelsPoster] dimension_parity_benign_skip '
            'prevSize=${currentW}x$currentH '
            'newSize=${rawWidth}x$rawHeight '
            'gated=${width}x$height action=$action',
          );
        }
        if (isZero || isSame) {
          return;
        ...
        AndroidSurfaceSizeGate.recordPainted(width, height);
